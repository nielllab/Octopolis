---
title: "Standard Norm of HiSeq with updated names 09/13/20"
author: "jos"
date: "9/17/2020"
output: 
  html_document:
...
for (i in 1:length(all_list)){
  #all_list[[i]][["percent.mt"]] <- PercentageFeatureSet(all_list[[i]], pattern = "^mitochondria-") #preferred for for loop over original command
   all_list[[i]] <- NormalizeData(all_list[[i]])
   all_list[[i]] <- FindVariableFeatures(all_list[[i]], selection.method = "vst") #omit nfeatures = 2000 for 9120, 9320, 91320 dataset
}
...
## Identify anchors and integrate datasets.
all.anchors <- FindIntegrationAnchors(object.list = all_list, anchor.features = 3000, verbose = TRUE) # can omit dims = 1:30 but takes a really long time to run; determine if want to keep anchor.features = 2000; anchor.features = 3000 for 91320 data; omit ndims for 9120, 9320, 91320 data 
allnorm.integrated <- IntegrateData(anchorset = all.anchors, verbose = TRUE)
...
```{r, echo = FALSE, eval = FALSE } 
## Subset nFeature > 500
# omit for 91320 dataset since min.features = 750
...
```{r echo = FALSE, eval = FALSE}
## Subset nFeature > 750
# subset min.features = 750, omit for 91320 dataset since min.features = 750
all <- subset(all, subset = nFeature_RNA > 750 & nFeature_RNA < 7000 & nCount_RNA <30000)
# eval features after subsetting: 
VlnPlot(all, features = c("nFeature_RNA", "nCount_RNA"), pt.size = 0, y.max = 30000) + NoLegend() 
