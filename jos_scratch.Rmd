...
addGeneNamesTable<- function(data,gene_master){
  ...
  for (i in 1:nrow(data)){
    ...
    ggene = data$geneID[[i]] # individual cluster markers
...
```{r, echo = FALSE, eval = TRUE}
replaceLabelsY <- function(p,oldylabels,newylabels){
  pp<- ggplot_build(p)
  oldylabels = ggplot_build(p)$layout$panel_params[[1]]$y$breaks
  newylabels = oldylabels
  for (i in 1:length(oldylabels)){
      loc<-which(genelabels==(oldylabels[[i]])) # i/ind 4 = 919, G34789, neuroendocrine protein/ label = paste(genelabels[919], "-", namelist[919]) 
      if (length(loc)> 0) {
        #loc<-which(oldylabels %in% genelabels)
        newylabels[[i]] = paste(oldylabels[[i]], "-", namelist[[loc[1]]])
         #} else {
      #newylabels[[i]] = paste(oldylabels[[i]])
}}
...
for (i in 1:50){
  ...
  #p<- p + scale_y_discrete(labels = newylabels) # this works?
  p <- replaceLabelsY(p, oldylabels, newylabels)
  features<-addGeneNamesTable(p$data,yfg)
  print(p)
  print(features)
}
...
for (m in 1:(nlevels((all.tree$seurat_clusters)) - 1)){
  ...
  clustermarkers_i %>%
   ...
   top30<-addGeneNamesTable(top30,yfg)
   
   print(top30)
  p<-DotPlot(all.tree,features=top30$geneID) + RotatedAxis() + theme(axis.text.x = element_text(size = 6))
  #p<-replaceLabelsX(p,genelabels,namelist)
  print(p)  
}
...
```{r, echo = FALSE, eval = TRUE}
replaceLabelsXx <- function(p,oldxxlabels,newxxlabels){
  pp<- ggplot_build(p)
  oldxxlabels = ggplot_build(p)$layout$panel_params[[1]]$x$breaks
  newxxlabels = oldxxlabels
  for (i in 1:length(oldxxlabels)){
      loc<-which(genelabels==(oldxxlabels[[i]])) # i/ind 4 = 919, G34789, neuroendocrine protein/ label = paste(genelabels[919], "-", namelist[919]) 
      if (length(loc)> 0) {
        #loc<-which(oldxxlabels %in% genelabels)
        newxxlabels[[i]] = paste(oldxxlabels[[i]], "-", namelist[[loc[1]]])
         #} else {
      #newxxlabels[[i]] = paste(oldxxlabels[[i]])
}}
 p<- p + scale_x_discrete(labels = newxxlabels)
}
...
for (m in 6:6){
  ...
  clustermarkers_i %>%
   ...
   print(top30)
  p<-DotPlot(all.tree,features=top30$geneID) + RotatedAxis() + theme(axis.text.x = element_text(size = 6))
  p<-replaceLabelsXx(p,oldxxlabels,newxxlabels)
  print(p)  
}
```
