...
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_heatmap.pdf",
...
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_vizdim.pdf",
    width = 8, # The width of the plot in inches
    height = 8)
for (i in 1:50){
...
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_umap.pdf",
    width = 8, # The width of the plot in inches
    height = 5)
...
```{r, echo = TRUE}
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_hierarchicaltree_umap.pdf",
    width = 8, # The width of the plot in inches
    height = 5)
...
```{r, echo = FALSE, eval = TRUE}
DefaultAssay(all.tree) <- "SCT"
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_top30clustersInclNoIDdot_sctassay.pdf",
    width = 8, # The width of the plot in inches
    height = 8)
for (m in 1:(nlevels((all.tree$seurat_clusters)) - 1)){ #(nlevels((all.tree$seurat_clusters)) - 1)
  ...
  clustermarkers_i %>%
   ...
   top30<-addGeneNamesTable(top30,top30$geneID,gene_master$ggene, gene_master$merge_desc)
     clusterName = paste("/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/", "cluster", m, "_top30", ".csv", sep = "")
...
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_treeclustermarkersnodesdot_allSctAssay.pdf",
    width = 8, # The width of the plot in inches
    height = 8)
PlotClusterTree(all.tree, label = TRUE)
...
for (n in 1:length(nodes)){ 
  ...
  nodemarkersROC %>%
    top_n(n = 30, wt = power) ->top30node
  top30node<-addGeneNamesTable(top30node,top30node$geneID,gene_master$ggene, gene_master$merge_desc)
  nodeName = paste("/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/", "node", n+45, "_top30", ".csv", sep = "")
  write.csv(top30node, nodeName)
  
   nodemarkersROC %>%
    top_n(n = -30, wt = power) ->bottom30node
  bottom30node<-addGeneNamesTable(bottom30node,bottom30node$geneID,gene_master$ggene, gene_master$merge_desc)
  nodeName = paste("/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/", "node", n+45, "_bottom30", ".csv", sep = "")
  ...
  q <- q+ ggtitle(paste('top30 for node', n+45, sep = " ")) 
  print(q)  
  ...
  p<-replaceLabelsX(p,gene_master$ggene,gene_master$merge_desc)
    p <- p+ ggtitle(paste('bottom30 for node', n+45, sep = " ")) 
  print(p)  
}
...
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_yfg-clustermarkers_dotfeat.pdf", #change each time
    width = 10, # The width of the plot in inches
    height = 8)
cl<-DotPlot(all.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
# The following requested variables were not found: G4907, G15066, G1766
...
noID$gene <-''
for (g in 1:length(genes)){
  loc = which(gene_master$ggene == genes$gene[[g]])
...
pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/sct_subsettingplots_noOrigIdent_umap.pdf",
    width = 8, # The width of the plot in inches
    height = 5)
...
#sigpc = c(1:12, 15, 16, 18, 19, 20, 21, 22, 24) #significant pcs

all2 <- RunPCA(all2, verbose = FALSE)

pdf(file = "/Users/josongco/Desktop/ABD/FinalSeqData/hifiasm5cells_092121names/umaploops_clustermarkers.pdf",
    width = 8, # The width of the plot in inches
    height = 5)

for (l in 2:12){
  
  pc1_12 <- RunUMAP(all2, dims = 1:l, verbose = FALSE)
  pc1_12 <- FindNeighbors(pc1_12 , dims = 1:l, verbose = FALSE)
  pc1_12 <- FindClusters(pc1_12 , verbose = FALSE)
  pc1_12.tree <- BuildClusterTree(pc1_12 , reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = 1:l)
  DimPlot(pc1_12.tree, reduction = "umap", label = TRUE) + plot_annotation(title = paste('umap for 1:', l, 'PCs', sep=""))
  PlotClusterTree(pc1_12.tree, label = TRUE) 
  
  genelist <- genesofint$ggene
  genelist <- addGeneNamesTable(genesofint,genesofint$ggene,gene_master$ggene, gene_master$merge_desc)
  
  cl<-DotPlot(pc1_12.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
  cl <- replaceLabelsX(cl,gene_master$ggene,gene_master$merge_desc)
  cl <- cl+ ggtitle('clustermarkers') # change each time
  print(cl)}
  
  pc15 <- RunUMAP(all2, dims = c(1:12, 15), verbose = FALSE)
  pc15 <- FindNeighbors(pc15 , dims = c(1:12, 15), verbose = FALSE)
  pc15 <- FindClusters(pc15 , verbose = FALSE)
  pc15.tree <- BuildClusterTree(pc15 , reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = c(1:12, 15))
  DimPlot(pc15.tree, reduction = "umap", label = TRUE) + plot_annotation(title = 'umap for 1:12, 15 PCs')
  PlotClusterTree(pc15.tree, label = TRUE) 
  
  clu<-DotPlot(pc15.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
  clu <- replaceLabelsX(clu,gene_master$ggene,gene_master$merge_desc)
  clu <- clu + ggtitle('clustermarkers') # change each time
  print(clu)
  
  pc16 <- RunUMAP(all2, dims = c(1:12, 15:16), verbose = FALSE)
  pc16 <- FindNeighbors(pc16 , dims = c(1:12, 15:16), verbose = FALSE)
  pc16 <- FindClusters(pc16 , verbose = FALSE)
  pc16.tree <- BuildClusterTree(pc16 , reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = c(1:12, 15:16))
  DimPlot(pc16.tree, reduction = "umap", label = TRUE) + plot_annotation(title = 'umap for 1:12, 15:16 PCs')
  PlotClusterTree(pc16.tree, label = TRUE) 
  
  clus<-DotPlot(pc16.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
  clus <- replaceLabelsX(clus,gene_master$ggene,gene_master$merge_desc)
  clus <- clus+ ggtitle('clustermarkers') # change each time
  print(clus)
  
  pc18 <- RunUMAP(all2, dims = c(1:12, 15:16, 18), verbose = FALSE)
  pc18 <- FindNeighbors(pc18 , dims = c(1:12, 15:16, 18), verbose = FALSE)
  pc18 <- FindClusters(pc18 , verbose = FALSE)
  pc18.tree <- BuildClusterTree(pc18 , reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = c(1:12, 15:16, 18))
  DimPlot(pc18.tree, reduction = "umap", label = TRUE) + plot_annotation(title = 'umap for 1:12, 15:16, 18 PCs')
  PlotClusterTree(pc18.tree, label = TRUE) 
  
  clust<-DotPlot(pc18.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
  clust <- replaceLabelsX(cluste,gene_master$ggene,gene_master$merge_desc)
  clust <- clust+ ggtitle('clustermarkers') # change each time
  print(clust)

  pc19 <- RunUMAP(all2, dims = c(1:12, 15:16, 18:19), verbose = FALSE)
  pc19 <- FindNeighbors(pc19 , dims = c(1:12, 15:16, 18:19), verbose = FALSE)
  pc19 <- FindClusters(pc19 , verbose = FALSE)
  pc19.tree <- BuildClusterTree(pc19 , reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = c(1:12, 15:16, 18:19))
  DimPlot(pc19.tree, reduction = "umap", label = TRUE) + plot_annotation(title = 'umap for 1:12, 15:16, 18:19 PCs')
  PlotClusterTree(pc19.tree, label = TRUE) 
  
  cluste<-DotPlot(pc19.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
  cluste <- replaceLabelsX(cluste,gene_master$ggene,gene_master$merge_desc)
  cluste <- cluste+ ggtitle('clustermarkers') # change each time
  print(cluste)
...
for (loop in 24:25){
  ...
  pc24_25 <- RunUMAP(all2, dims = 24:25, verbose = FALSE)
  pc24_25 <- FindNeighbors(pc24 , dims = 24:25, verbose = FALSE)
  pc24_25 <- FindClusters(pc24 , verbose = FALSE)
  pc24_25.tree <- BuildClusterTree(pc24_25 , reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = 24:25)
  DimPlot(pc24_25.tree, reduction = "umap", label = TRUE) + plot_annotation(title = paste('umap for', loop, 'PCs', sep=" "))
  PlotClusterTree(pc24_25.tree, label = TRUE) 
  
  cluster<-DotPlot(pc24_25.tree,features=rev(genelist$ggene)) + RotatedAxis()+ theme(axis.text.x = element_text(size = 6)) + NoLegend() 
  cluster <- replaceLabelsX(cluster,gene_master$ggene,gene_master$merge_desc)
  cluster <- cluster+ ggtitle('clustermarkers') # change each time
  print(cluster)}
