...
cluster.markers %>%
    group_by(cluster) %>%
    top_n(n = 2, wt = cluster.markers$diff_exp)# not working currently - "Error: Result must have length 266, not 5457" 
...
cluster.markers$GeneName <- '' # create empty column 
cl_inds <- which(cluster.markers[[7]] %in% yfg[[3]]) # find indices for TRUE geneIDs in selected csv/Genelist
cluster.markers$GeneName[cl_inds] <- yfg[which(yfg[[3]] %in% cluster.markers[[7]]),1] # include geneNames in markers file , take first index
```
