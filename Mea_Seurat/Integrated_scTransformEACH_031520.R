...
# Data Set 1: Project "OctoSeq1_MTnorm4"
...
raw1 <- CreateSeuratObject(counts = raw1, project = "OctoSeq1_MTnorm4", min.cells = 3, min.features = 200)
raw1
...
raw1.norm <- PercentageFeatureSet(raw1, pattern = "^mitochondria-", col.name = "percent.mt")
raw1.norm <- SCTransform(raw1.norm, vars.to.regress = c("nCount_RNA", "percent.mt"), verbose = FALSE)

saveRDS(raw1, file = "/Users/Hannah\ Bishop/Desktop/MeaS/Seurat/OSr1MTnorm4.rds")
...
# Data Set 2: Project "OctoSeq2_MTnorm4"
...
raw2 <- CreateSeuratObject(counts = raw2, project = "OctoSeq2_MTnorm4", min.cells = 3, min.features = 200)
raw2
...
raw2.norm <- PercentageFeatureSet(raw2, pattern = "^mitochondria-", col.name = "percent.mt")
raw2.norm <- SCTransform(raw2.norm, vars.to.regress = c("nCount_RNA", "percent.mt"), verbose = FALSE)

saveRDS(raw2, file = "/Users/Hannah\ Bishop/Desktop/MeaS/Seurat/OSr2MTnorm4.rds")
...
# Data Set 3: Project "OctoSeq3_MTnorm4"
...
raw3 <- CreateSeuratObject(counts = raw3, project = "OctoSeq3_MTnorm4", min.cells = 3, min.features = 200)
raw3
...
raw3.norm <- PercentageFeatureSet(raw3, pattern = "^mitochondria-", col.name = "percent.mt")
raw3.norm <- SCTransform(raw3.norm, vars.to.regress = c("nCount_RNA", "percent.mt"), verbose = FALSE)

saveRDS(raw3, file = "/Users/Hannah\ Bishop/Desktop/MeaS/Seurat/OSr3MTnorm4.rds")
...
allOSnorm <- merge(x = raw1.norm, y = c(raw2.norm, raw3.norm), add.cell.ids = c("run1", "run2", "run3"), project = "OSallNormMT2", merge.data = TRUE)
...
all.norm <- PercentageFeatureSet(allOSnorm, pattern = "^mitochondria-", col.name = "percent.mt")
all.norm <- SCTransform(all.norm, vars.to.regress = c("nCount_RNA", "percent.mt"), verbose = FALSE)

all.norm <- RunPCA(all.norm, features = VariableFeatures(object = all.norm), npcs = 200)
...
all.norm <- saveRDS(all.norm, file = "/Users/Hannah\ Bishop/Desktop/MeaS/Seurat/allOSnorm_merged4.rds")
