...
datafiles <- read.csv("/Users/josongco/Desktop/SeqData/OSdirectories.csv", stringsAsFactors = FALSE)
str(datafiles) # view the structure of the data frame

# Parameters: 
x = 1 # if x is set to 1, data will be from runs 1:3 using original gtf file with ocbim; otherwise, data will be from runs 4:6 using the HiSat gtf file with OCTOgene
...
# for now, manually create individual seurat objects to merge and apply std or SCT normalization on individual datasets embedded within a merged object 
all1 <- all_list[[1]]
all2 <- all_list[[2]]
all3 <- all_list[[3]]

# save out so in the future you can just load these datasets (no normalization, only subset min cells and min features)
saveRDS(all1, "~/all1nonorm.RDS")
saveRDS(all2, "~/all2nonorm.RDS")
saveRDS(all3, "~/all3nonorm.RDS")

# start here to load RDS files with min cells = 3 and min features = 200 instead of starting with raw counts from Talapas
all1 <- readRDS(file = "/Users/josongco/Desktop/SeqData/all1nonorm.RDS")
all2 <- readRDS(file = "/Users/josongco/Desktop/SeqData/all2nonorm.RDS")
all3 <- readRDS(file = "/Users/josongco/Desktop/SeqData/all3nonorm.RDS")

all <- merge(x = all1, y = c(all2, all3), add.cell.ids = c("Run1", "Run2", "Run3"), project = "OrigGTF", merge.data = FALSE) 
...
# save out so in the future you can just load these datasets (no normalization, only subset min cells and min features)
saveRDS(all4, "~/all4nonorm.RDS")
saveRDS(all5, "~/all5nonorm.RDS")
saveRDS(all6, "~/all6nonorm.RDS")

# start here to load RDS files with min cells = 3 and min features = 200 instead of starting with raw counts from Talapas
all4 <- readRDS(file = "/Users/josongco/Desktop/SeqData/all4nonorm.RDS")
all5 <- readRDS(file = "/Users/josongco/Desktop/SeqData/all5nonorm.RDS")
all6 <- readRDS(file = "/Users/josongco/Desktop/SeqData/all6nonorm.RDS")

all <- merge(x = all4, y = c(all5, all6), add.cell.ids = c("HSrun1", "HSrun2", "HSrun3"), project = "HiSatGTF", merge.data = FALSE) 
