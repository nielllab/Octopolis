...
datafiles <- read.csv("~/GDrive/miller niell octo seq/project data/scRNAseq/Seurat/directoryMaster_070220.csv", stringsAsFactors = FALSE)
...
for(f in 1:2){
    all <- Read10X(data.dir = data_dir[f+8])
   ...
   all <- CreateSeuratObject(counts = all, project = paste("HiSeqTrimRun", f, sep = ""), min.cells = 3, min.features = 500)
  all_list[[f]] = all    
}

saveRDS(all_list, "/Users/josongco/Desktop/FinalSeqData/HiSeq_orig.RDS")
...
all <- subset(all, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5) # set parameters to include cells with the following criteria: > 200, < 2500, less than 5% mitochondrial genes, since a higher number of mitochondrial genes is indicative of stress 
  ...
  ref <- read.csv("/Users/josongco/Desktop/SeqData/cov4_len200_splice3_strandness_gtf_namekey_singled_edited.csv", stringsAsFactors = FALSE)
    for(f in 1:3){
    all <- Read10X(data.dir = data_dir[f+3])
