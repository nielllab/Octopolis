...
# alternative: create csv file holding directory information and turn the csv file into a data frame after reading it in
...
Note: options for which raw data (old vs new), options for standard pre-processing (source: https://satijalab.org/seurat/v3.1/pbmc3k_tutorial.html) vs scTransform (source: https://satijalab.org/seurat/v3.0/sctransform_vignette.html)
```{r}
if(x = 1){
for(f in 1:3){
    data_dir <- datafiles$Directory
    list.files(data_dir)
...
all <- ScaleData(all, vars.to.regress = "nCount_RNA", "percent.mt") # regress out UMI counts and percent.mt which is helpful when integrating v2 and v3 chemistry; alt: all <- ScaleData(all, features = all.genes)
  ...
  all <- SCTransform(all, vars.to.regress = c("nCount_RNA", "percent.mt")) # not from the link above!!
}
