...
```{r, echo = FALSE}
# to knit document, use the following command: rmarkdown::render("/Users/josongco/Documents/GitHub/Octopolis/OSall_30pcs30dimsres1_normCCA_HiSat.Rmd") ~30m
...
all_list <- readRDS("~/hisatAll_list.rds")
for (i in 4:length(all_list)){
  ...
  all_list[[i]] <- ScaleData(all_list[[i]], vars.to.regress = "percent.mt") # ~30m, ~50m, ~40m # cannot regress out percent.mt
  all_list[[i]] <- FindVariableFeatures(all_list[[i]], selection.method = "vst", nfeatures = 2000)

saveRDS(all_list, "/Users/josongco/Desktop/SeqData/allnormCCA_Hisat_notinteg.rds")
all_list <- readRDS("/Users/josongco/Desktop/SeqData/allnormCCA_Hisat_notinteg.rds")

allnorm.anchors <- FindIntegrationAnchors(all_list[4:6], dims = 1:30) # runs CCA ~30m; shows 0% but moves on to merging objects etc; found 43006 anchors -> 10257 [what is the ideal/typical number for anchors across three datasets?]; overall takes ~2-3hr
allnorm.integrated <- IntegrateData(anchorset = allnorm.anchors, dims = 1:30)

saveRDS(allnorm.integrated, "/Users/josongco/Desktop/SeqData/allnormCCA_Hisat.rds")
integrated <- readRDS("/Users/josongco/Desktop/SeqData/allnormCCA_Hisat.rds")
```
...
```{r, echo = FALSE}
all <- readRDS("/Users/josongco/Desktop/SeqData/allnormCCA_Hisat.rds")
