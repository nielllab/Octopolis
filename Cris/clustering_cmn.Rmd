...
```{r}
ref <- read.csv("D:/data/octo seq/refMaster.csv",stringsAsFactors=FALSE)
...
DotPlot(all.normTREE,features=rownames(these))
...
DotPlot(all.normTREE,features=rev(all.genes[genelist])) + RotatedAxis()+ theme(axis.text.x = element_text(size = 4))
...
DotPlot(all.normTREE,features=all.genes[cadh[1:100]]) + RotatedAxis()+ theme(axis.text.x = element_text(size = 4))

DotPlot(all.normTREE,features=all.genes[cadh[101:193]]) + RotatedAxis()+ theme(axis.text.x = element_text(size = 4))
