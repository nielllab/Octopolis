...
```{r} 
# chunk (editing) in progress 9/3/20 12:07am
# nlevels(all.tree$seurat_clusters)
markers_list = list()
for (m in 1:2){
  ...
  markersROC$header[m] <- paste("Cluster", all.tree$seurat_clusters[m], sep = " ")
  markersROC$top5markers[m] <- paste((head(markersROC[m]$geneID, n = 5)))
  marker_output <- c(header, top5markers)
  print(marker_output)
  #topfeatures[[m]] <- head(markers_list[[m]][["geneID"]], n = 5) 
...
for (n in 1:2){
  ...
  nodefeatures_df <- data.frame(Col1 = rep(nodes[n], 5), Col2 = top5nodemarkers[n]) 
        print(head(nodefeatures_df))
        nodefeatures_df.merged <- nodefeatures_df[n] %>%
            dplyr::group_by(Col1) %>%
  ...
  #nodefeatures_df <- data.frame(nodes[[2]], (headtopnodefeatures[[2]], n = 5))
  DotPlot(all.tree, features = nodefeatures_df.merged$Col2)+ RotatedAxis() # can ID which node to view genes
}
