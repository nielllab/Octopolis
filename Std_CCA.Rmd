...
id_cell <- table(Idents(all2))
barplot(id_cell, main = "cell # in each cluster", xlab = "cluster ID")
table(all2$orig.ident)
prop.table(table(Idents(all2)))
id_runs <- table(all2$orig.ident, Idents(all2))
...
all2.tree <- BuildClusterTree(all2, reorder = TRUE, reorder.numeric = TRUE, slot = "scale.data", verbose = TRUE, dims = DM)
PlotClusterTree(all2.tree, label = TRUE)
all2.tree <- RunUMAP(all2.tree, dims = DM)
DimPlot(all2.tree, reduction = "umap", label = TRUE)

id_cell <- table(Idents(all2.tree))
barplot(id_cell, main = "cell # in each cluster", xlab = "cluster ID")
table(all2.tree$orig.ident)
prop.table(table(Idents(all2.tree)))
id_runs <- table(all2.tree$orig.ident, Idents(all2.tree))
#barplot(id_runs, main = "relationship between cluster ID and run representation", xlab = "cluster ID", ylab = "cells", col = c("lightcoral", "palegreen3", "steelblue1"), legend = rownames(id_runs), beside=TRUE) # all runs
barplot(id_runs, main = "relationship between cluster ID and run representation", xlab = "cluster ID", ylab = "cells", col = c("lightcoral", "steelblue1"), legend = rownames(id_runs), beside=TRUE) # v3 chem

VlnPlot(all2, features = "nCount_RNA", y.max = 20000, pt.size = 0.001, ncol = 1)
VlnPlot(all2, features = "nFeature_RNA", y.max = 20000, pt.size = 0.001, ncol = 1)
```
...
VlnPlot(all.tree, features = "nCount_RNA", y.max = 20000, pt.size = 0.001, ncol = 1)
VlnPlot(all.tree, features = "nFeature_RNA", y.max = 20000, pt.size = 0.001, ncol = 1)
```
...
for (m in 1:nlevels(all.tree$seurat_clusters)){
  ...
  paste("Cluster", m, sep = "")
  print(head(markersROC$geneID, n = 5))
}

#test 
#cluster2.markersROC <- FindMarkers(all.tree, ident.1 = 2, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)
#write.csv(cluster2.markersROC, "/Users/josongco/Desktop/FinalSeqData/cluster2markers15pc.csv")

#print(head(markersROC, max.levels = 5))
#markersROC <- setNames(cbind(rownames(markersROC), markersROC, row.names = NULL), 
 #       c("geneID", "myAUC", "avg_diff", "power", "pct.1", "pct.2"))
#markersROC$diff_exp <- (markersROC$pct.1/markersROC$pct.2)
#markersROC_sort <- markersROC[order(-markersROC$diff_exp),]
#print(head(markersROC$geneID, max.levels = 5))
#row.names(markers_list[[1]])
```
